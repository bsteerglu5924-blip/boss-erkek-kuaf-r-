-- ============ book_appointment: style_analysis baglantisi ============
-- Imza degisiyor (p_style_analysis_id eklendi), eski surum drop edilmeli
...
begin
  ...
  -- Stil analizi baglantisi opsiyonel ve ikincil: basarisiz olsa/eslesmese
  -- bile ana randevu islemi asla etkilenmemeli.
  if p_style_analysis_id is not null then
    update style_analyses
    set appointment_id = v_new_id, updated_at = now()
    where id = p_style_analysis_id
...
grant execute on function public.book_appointment(uuid, uuid, timestamptz, text, text, text, text, uuid) to anon, authenticated, service_role;
